...
- [ ] 从设置中心移除或弱化 `帮助` 分区，将帮助入口改为全局辅助菜单中的外部链接。
- [ ] 从设置中心移除或弱化 `反馈` 分区，将反馈入口改为全局辅助菜单中的外部链接。
- [x] 增加 `关于` 分区，展示产品名、版本、许可证、仓库地址和本地运行信息。
- [x] 将主导航右侧设置入口改为可收起的圆形 `...` 更多菜单。
- [ ] 更多菜单展开后包含：状态、活动记录、帮助、反馈、论坛、设置。
- [ ] `帮助`、`反馈`、`论坛` 短期统一跳转到 `https://kezhongke.cn/`，并以新标签页打开。
- [ ] `活动记录` 继续进入 `/settings?section=activity`，`设置` 进入 `/settings?section=ai`，避免外部支持入口和内部设置入口混淆。
